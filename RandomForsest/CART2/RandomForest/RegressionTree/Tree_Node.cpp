...
Tree_Node::Tree_Node(map<string, vector<string>> temp_Table,vector<string> temp_Attribute)
{
    ...
    VarianceValue_CurrentNode = Calculation_VarianceValue(Table_Data);//计算当前节点方差值
...
void Tree_Node::generate_ChildNode()
{
    ...
    if (VarianceValue_CurrentNode <= 0.02)//第一种情况，判定时不时所有类别相同以后应该变更为方差小于阙值然后平均数做为叶子节点
    {
        stringstream ss;
        string Average_tmp;
...
string Tree_Node::findBestAttribute()
{
    ...
    double Variance_BestMin = VarianceValue_CurrentNode;//最低基尼指数
    vector<string> AttDataTable_tmp;//去除类别后的属性集合
    AttDataTable_tmp = FullAtt_DataTable;
    AttDataTable_tmp.pop_back();
    for(int i = 0; i < AttDataTable_tmp.size(); i++)
...
void Tree_Node::FindBestDivideSign(string Attribute,double* BestSign_Divide,double* Variance_BestSignDivide)
{
    ...
    double BestSign_Divide_tmp;
    map<string,vector<string>> TableAttValue_tmp;//属性和取值对应表感觉有优化的余地，这个应该没有必要存在，现在头脑不好用，以后修改
    for(auto iter = Table_Data.rbegin();iter != Table_Data.rend();iter++)//不需要修改，这个是为了去除重复取值  
    {
...
double Tree_Node::Calculation_VarianceValue(map<string, vector<string>> Table_Data)//假如是空集基本不会..计算的是输入的集合GIni值
{
    if(Table_Data.empty())
        return 0;
...
void Tree_Node::PrintDecisionTree(Tree_Node* root)
{
    ...
    while(!Queue_TreeNode.empty())
    {
        ...
        cout << "当前节点Average值： " << Node_Current->AverageValue_CurrentNode << endl;
        cout << "当前节点Variance值： " << Node_Current->VarianceValue_CurrentNode << endl;
