...
void Generate_ChildTreeSampleFile(int K,int N,vector<string> Feature_Set,map<string,vector<string>> Data_Set,map<int,map<int,int>>& Numsign_RanForestTrain_tmp,map<int,map<string,vector<string>>>& data_TableRanForest_tmp,map<int,vector<string>>& Atts_RanForest_tmp);//K是每棵子树特征数目，N是子树数目修改后让它在生成数据的时候直接存入对应的map里面，降低计算机消耗
...
void Generate_ChildTreeSampleFile(int K,int N,vector<string> Feature_Set,map<string,vector<string>> Data_Set,map<int,map<int,int>>& Numsign_RanForestTrain_tmp,map<int,map<string,vector<string>>>& data_TableRanForest_tmp,map<int,vector<string>>& Atts_RanForest_tmp)
{
    ...
    for(int i = 0; i < N; i++)//每次循环结束需要清理tmp集合里面的数据
    {
        ...
        Atts_RanForest_tmp[i] = Features_ChildTree;
