...
*       20200919 之前随机森林的算法错了，随机选择特征应该在结点选择.
*                                                                           *
*                                                                           *
*                                                                           *
***************************************************************************/ 
...
void Tree_Node::generate_ChildNode(int K)
{
    vector<string> AttDataTable_tmp;//去除类别后的属性集合
    ...
    if (AttDataTable_tmp.size() < K)//假如特征值小于K，那么选择类型最多的做为当前特征
    {
        map<string,int>::iterator it = ClassSample_CurrentNode.begin();
    ...
    for (int i = 0; i < AttDataTable_tmp.size(); i++)//第二种情况判定特征取值是否相同
    {
    ...
    vector<string> AttSet_tmp = Choose_RandomFeatures(FullAtt_DataTable,K);//这里采用随机生成的办法随机选择K个特征。 
    for(int i = 0; i < AttSet_tmp.size(); i++)
        cout << AttSet_tmp[i] <<"---";
    cout << endl;
    Attribute_CurrentNode = findBestAttribute(AttSet_tmp);
...
//寻找最优划分
string Tree_Node::findBestAttribute(vector<string> AttSet_tmp)
{
    string BestAtt;//最佳属性
    double BestSign_Divide;
    double Gini_BestMin = 1;//最低基尼指数
    vector<string> AttDataTable_tmp;//去除类别后的属性集合
    AttDataTable_tmp = AttSet_tmp;
    AttDataTable_tmp.pop_back();
...
vector<string> Tree_Node::Choose_RandomFeatures(vector<string> Feature_Set, int K)
{
    //sleep(3);//因为这个函数会被多次调用，所以需要sleep来重置初始种子也可以将种子放到循环外，就不内聚了
    int NumAtt_tmp = Feature_Set.size() - 1;
    ...
    for(int i = 0; i < Numflag_tmp.size(); i++)
    {
        ...
        string Att_selected = Feature_Set[Num_selected];
        FeatureRandom_tmp.push_back(Att_selected);
    }
    FeatureRandom_tmp.push_back(Feature_Set[NumAtt_tmp]);//添加样本类型
...
string Tree_Node::PredictionSampleClass(map<string,string> Set_AttSample)
{
    if(childNode.size() == 0)
    {
        //cout << "返回结果类型: " << Attribute_CurrentNode << endl;
    ...
    else
    {
        ...
        {
            //cout <<"属性: "<< Attribute_CurrentNode << "取值为: " << Set_AttSample[Attribute_CurrentNode] << "<= 当前节点最佳划分" << BestSignDivide_CurrentNode << "True"<<  endl; 
            string signless_tmp = "<";
            string string_tmp;
            string AttValue_tmp;
            stringstream ss;
        ...
        {
            //cout <<"属性: "<< Attribute_CurrentNode << "取值为: " << Set_AttSample[Attribute_CurrentNode] << "> 当前节点最佳划分" << BestSignDivide_CurrentNode << "False"<< endl;
            string signplus_tmp = ">";
            string string_tmp;
            string AttValue_tmp;
            stringstream ss;
...
void Tree_Node::PrintDecisionTree(map<string,double>& FeatureGini,Tree_Node* root)
{
