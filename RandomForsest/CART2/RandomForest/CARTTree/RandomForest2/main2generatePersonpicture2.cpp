...
int main()
{
    ...
    outfile.close();
    */vector<string> Atts_tmp = FeatureSet_tmp;
    Atts_tmp.pop_back();
    map<int,map<string,string>> TwoFeature_tmp = Generate_TwoFeature(Atts_tmp);
    map<string,double> Person_TwoFeature;
    for(int i = 0; i < TwoFeature_tmp.size(); i++)
    {
        cout <<"第" << i <<"对特征组合" << endl;
        auto iter = TwoFeature_tmp[i].rbegin();
        string FeatureFirst_tmp = iter->first;
        string FeatureSecond_tmp = iter->second;
        string Feature_tmp = FeatureFirst_tmp + "-" + FeatureSecond_tmp;
        double Num_Pearson = Calcu_PearsonCorrelationCoefficient(TwoFeature_tmp[i],DataSet_tmp);
        Person_TwoFeature[Feature_tmp] = Num_Pearson;
        cout << Feature_tmp <<": " << Num_Pearson << endl;
    }
    map<string,double> FeatureGini = Person_TwoFeature;
    vector<pair<string, double>> VectorMap_tmp;
    for (auto it = FeatureGini.begin(); it != FeatureGini.end(); it++)
        VectorMap_tmp.push_back(make_pair(it->first, it->second));
    sort(VectorMap_tmp.begin(), VectorMap_tmp.end(),
    ...
    ofstream outfile;
    outfile.open("Person.csv");
    outfile << "Features,"<<"PerSon"<<endl;
    int i_tmp = 0;
    for(auto it = VectorMap_tmp.rbegin(); it != VectorMap_tmp.rend(); it++)
    {
        if(i_tmp < 10)
            outfile << it->first <<"," << it->second << 10 << i_tmp << endl;
        else
            outfile << it->first <<"," << it->second << 10 << i_tmp << endl;
        if(i_tmp >= 15)
            break;
