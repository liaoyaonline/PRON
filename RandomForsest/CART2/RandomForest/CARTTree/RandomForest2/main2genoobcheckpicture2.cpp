...
{
	//输入
    string NameDataFile = "OOBdata.txt";
    ...
    map<string,vector<string>> DataSet_tmp = TableData_tmp;//样本数据集
    ...
    for(int i = 0; i < Num_Sample; i++)//将Train保存进入Traindata中
    {
        ...
        for(int m = 1; m < FeatureSet_tmp.size(); m++)
        {
...
void ReadSampleData(string NameDataFile,vector<string>& AttsDataFile_tmp,map<string, vector<string>>& TableDataFile_tmp)
{
    ...
    for(int i = 0; i < AttsDataFile_tmp.size(); i++)
        cout << AttsDataFile_tmp[i] << "---";
    cout << endl;
    for(auto iter = TableDataFile_tmp.rbegin(); iter != TableDataFile_tmp.rend(); iter++)
    {
        cout << iter->first << ": " << endl;
        for(auto it = iter->second.begin(); it != iter->second.end(); it++)
            cout << *it <<" ";
