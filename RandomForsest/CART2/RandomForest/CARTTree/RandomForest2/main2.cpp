...
int main()
{
    ...
    //在移除一些特征时，出现了段错误，等神志清醒时，需要测试哪里出问题了
    ...
    cout << endl;
    for(auto iter = DataSet_tmp.rbegin(); iter != DataSet_tmp.rend(); iter++)
    {
    ...
    vector<string> FeatureSet_tmp2;
    for(int i = 0; i < FeatureSet_tmp.size(); i++)
    {
        FeatureSet_tmp2.push_back(FeatureSet_tmp[i]);
    }
    Generate_ChildTreeSampleFile(N,FeatureSet_tmp2,DataSet_tmp,Numsign_RanForestTrain,data_TableRanForest);
    Generate_ChildTree(K,N,FeatureSet_tmp2,data_TableRanForest,RanForest);
    double ErrorRate_OOB = Check_RandomForest(N,DataSet_tmp);
