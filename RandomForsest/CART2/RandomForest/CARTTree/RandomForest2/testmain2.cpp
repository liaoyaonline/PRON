...
void Bootstrap_Sampling(vector<string> Atts_ChildData,map<string, vector<string>>TableData_Child);//输入特征集和样本集通过自取样后将Train和Test样本分别存入对应样本集
void Save_BootstrapSample(vector<string> Atts_ChildData,string FileName_ChildData);//将Train和Test的样本集保存进入数据文件
...
void Read_ChildTreeSampleFileAll(int N);//N是子树数目
...
void Generate_ChildTree(int N);//怎么读树或者说怎么调用
