...
*       20200908 生成绘制基分类器和OOB误差之间的图像数据后面在main3picture
*                里面做了*
*                                                                           *
***************************************************************************/ 
#include <iostream>
...
map<int,map<string,int>> ErrorRate_ChildTree;//子树准确率，临时
map<string,int> Precision_P;//临时，计算随机森林查准率
...
int main()
{
    ...
    */map<string,vector<string>> DataSet_tmp;//样本数据集
    ...
    {
        Generate_ChildTreeSampleFile(3,N,FeatureSet_tmp,DataSet_tmp);//添加输入为样本子集和特征子集
...
vector<string> LVM(map<string,vector<string>> Data_Set,vector<string> Feature_Set,int Num_Stop,int K)
{
    ...
    while(t < Num_Stop)
    {
        ...
        Generate_ChildTreeSampleFile(3,120,FeatureSet_tmp,DataSet_tmp);//添加输入为样本子集和特征子集
        Read_ChildTreeSampleFileAll(120,Numsign_RanForestTrain,data_TableRanForest,Atts_RanForest);//map每次更新就不必要做为输入了
        Generate_ChildTree(120,data_TableRanForest,Atts_RanForest,RanForest);
        double OOBErrorRate_tmp = Check_RandomForest(120,Data_Set);//返回OOB错误率
...
double Check_RandomForest(int N,map<string, vector<string>>TableData_test)
{
    ...
    for(int i = 0; i < NumSample_TestData; i++)
    {
        ...
        string Result_good = "good";//临时变量用来测算查准率
        ...
        if( Result_Correct == Result_Prediction )//临时，用于测算每颗子树准确率
